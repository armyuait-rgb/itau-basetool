...
stability smoke and release artifact verification on top of the Phase 2
cutover gates. Upstream sync and optional downstream staging are manual only.
...
| Downstream stage simulation (optional) | OK | Windows | `5889ff3` |
...
- **Downstream simulation (optional manual)** — extract tarball, run method matrix,
  graceful shutdown (SIGTERM on Linux/macOS, `terminate()` on Windows).
...
Optional manual downstream staging (not part of CI):
...
- Pass an explicit archive path after `build-release-artifact.py`. When omitted,
  verify picks the newest `dist/basetool-runner-*.tar.gz` by mtime.
...
| Stability / leak | `scripts/smoke/runner-stability-smoke.py` | Tag push; manual nightly workflow | **3 — live** |
| Release artifact | `scripts/release/verify-release-artifact.py` | Tag push | **3 — live** |
| Downstream stage | `scripts/release/simulate-downstream-stage.py` | Manual only | **optional** |
...
| Supply-chain audit | `pip-audit` in nightly workflow | Manual (`workflow_dispatch`) | **optional** |
...
- [`.github/workflows/nightly.yml`](../.github/workflows/nightly.yml) — manual stability smoke and pip-audit
...
`release.yml` runs the full CI matrix, stability smoke, build, verify, and
publishes a GitHub release. Tags containing `smoke` are published as
prereleases.
