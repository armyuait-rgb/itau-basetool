...
class Layer4(threading.Thread):
    ...
    def _update_stats(self, bytes_sent: int):
        ...
        if self.target_health and self.target_key and self.target_health.record_success(self.target_key):
            logger.info("target %s recovered: traffic flowing", self.target_key)
...
def health_watchdog_loop(stop_event: threading.Event, target_health: TargetHealth,
    ...
    while stop_event.is_set():
        ...
        for key in target_keys:
            ...
            last_error = str(entry.get("last_error", "") or "unknown")
            if successes > 0:
                continue
            if attempts <= 0:
                continue
            ...
            if now - prev >= interval:
                message = (
                    f"target {key} unreachable: 0/{attempts} attempts (last: {last_error})"
                )
...
class AttackManager:
    ...
    def start(self):
        ...
        self.health_thread = threading.Thread(
            ...
            args=(self.event, self.target_health, self.target_keys),
            daemon=True
        )
